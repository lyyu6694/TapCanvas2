...
export async function sendVerificationCode(
  ...
  const r = await apiFetch(`${API_BASE}/auth/email/send-code`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!r.ok) {
    const text = await r.text().catch(() => '')
    throw new Error(`send code failed: ${r.status} ${text}`.trim())
  }
  return r.json()
}
...
export async function emailSignup(
  ...
  const r = await apiFetch(`${API_BASE}/auth/email/signup`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!r.ok) {
    ...
    throw new Error(error?.error || `signup failed: ${r.status}`)
  }
  return r.json()
}
...
export async function emailLogin(
  ...
  const r = await apiFetch(`${API_BASE}/auth/email/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!r.ok) {
    const error = await r.json().catch(() => ({}))
    throw new Error(error?.error || `login failed: ${r.status}`)
  }
  return r.json()
}
...
export async function resetPassword(
  ...
  const r = await apiFetch(`${API_BASE}/auth/email/reset-password`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!r.ok) {
    const text = await r.text().catch(() => '')
    throw new Error(`reset password failed: ${r.status} ${text}`.trim())
  }
  return r.json()
}
